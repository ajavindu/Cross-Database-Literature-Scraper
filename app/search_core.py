...
# ===========================================================================
def _simplify_query(query, keep_boolean=True):
    """
    Make a PubMed-style query safe for keyword engines (OpenAlex / Crossref).
    Removes field tags like [Title/Abstract] and wildcards (* ?), which these
    APIs reject. OpenAlex understands AND/OR/NOT/quotes/parentheses, so those
    are kept (keep_boolean=True); Crossref does not, so they're dropped.
    """
    q = re.sub(r"\[[^\]]*\]", " ", query or "")   # strip [Mesh], [dp], etc.
    q = q.replace("*", "").replace("?", "")         # wildcards unsupported
    if not keep_boolean:
        q = re.sub(r"[()\"]", " ", q)               # drop grouping and quotes
        q = re.sub(r"\b(AND|OR|NOT)\b", " ", q)     # drop boolean operator words
...
def search_openalex(query, max_results, year_low, year_high, creds):
    ...
    search_q = _simplify_query(query, keep_boolean=True)
    ...
    while len(records) < max_results and cursor:
        params = {"search": search_q, "per-page": per_page, "cursor": cursor}
        if filters:
...
def search_crossref(query, max_results, year_low, year_high, creds):
    ...
    search_q = _simplify_query(query, keep_boolean=False)
    ...
    records, cursor = [], "*"
    while len(records) < max_results and cursor:
        params = {"query": search_q, "rows": rows, "cursor": cursor}
        if filters:
            params["filter"] = ",".join(filters)
        if email:
            params["mailto"] = email
