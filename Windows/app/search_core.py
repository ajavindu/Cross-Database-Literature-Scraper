...
def dedupe(records):
    ...
    Two records are treated as the same if they share a DOI OR a normalised
    title. Both keys are tracked, so a record that has a DOI still matches a
    copy of the same paper that only carries the title.
    Returns (unique_records, num_duplicates_removed).
    """
