...
from search_core import (
    UNIFIED_FIELDS, DATABASES, search_one, dedupe,
...
DB_ORDER = ["pubmed", "embase", "scopus", "serpapi"]
...
<body>
  ...
  <form method="post" action="/search">
    ...
    <div class="card">
      ...
      <p class="hint">Tick things to REMOVE from the results (by publication type). Applied to PubMed / Scopus / Embase.
         Google Scholar carries no type information, so these can't filter it.</p>
      <div class="grid">
...
def render(**kw):
    base = dict(
        ...
        defaults={"pubmed": config.ENABLE_PUBMED, "embase": config.ENABLE_EMBASE,
...
def index():
    defaults = {
        "pubmed": config.ENABLE_PUBMED, "embase": config.ENABLE_EMBASE,
        "scopus": config.ENABLE_SCOPUS, "serpapi": config.ENABLE_SERPAPI,
    }
...
def search():
    ...
    grand = None
    if len(results) > 1:
        grand_records, grand_dups = dedupe(all_kept)
    ...
    return render(
        ...
        creds=creds, results=results, grand=grand,
    )
...
def dedupe_tool():
    ...
    return render(dedupe_result=dedupe_result)
